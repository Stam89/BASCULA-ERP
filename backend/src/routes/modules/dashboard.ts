...
dashboardRouter.get("/", asyncRoute(async (_req, res) => {
  const [
    farmers,
    tickets,
    stock,
    ...
    pool.query("SELECT COUNT(*)::int AS count FROM weighing_tickets WHERE created_at::date = CURRENT_DATE"),
    pool.query("SELECT COALESCE(SUM(quantity), 0)::numeric AS quantity FROM inventory_stock WHERE ownership = 'OWNED'"),
    pool.query("SELECT COALESCE(SUM(balance), 0)::numeric AS amount FROM farmer_advances WHERE status IN ('CONFIRMED', 'PARTIAL')"),
    pool.query("SELECT COALESCE(SUM(balance), 0)::numeric AS amount FROM accounts_payable WHERE status IN ('CONFIRMED', 'PARTIAL')"),
    pool.query("SELECT COALESCE(SUM(total_amount), 0)::numeric AS amount FROM sales WHERE created_at::date = CURRENT_DATE"),
    pool.query(
      `SELECT *
       FROM cash_registers
       WHERE status = 'OPEN'
