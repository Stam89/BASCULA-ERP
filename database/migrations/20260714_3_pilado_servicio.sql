-- La MATRIZ (la piladora) es el accionista principal y presta el servicio de
-- secado + pilado a los otros accionistas, cobrando por quintal. Cada servicio
-- es un ingreso/cuenta por cobrar para la matriz y una cuenta por pagar para el
-- accionista al que se le procesó.

-- 1) Da un nombre genérico al accionista por defecto. Una instalación nueva
-- puede personalizarlo después desde Configuración sin cambiar el código.
UPDATE accionistas
SET name = 'MATRIZ', code = 'MATRIZ'
...
CREATE TABLE IF NOT EXISTS pilado_services (
  ...
  provider_accionista_id UUID NOT NULL REFERENCES accionistas(id),   -- matriz
